...
export default function TaskDetailPage({
    ...
    return (
      <RequireWallet>
        <div className="max-w-3xl mx-auto px-4 py-20 text-center">
          ...
          <Link href="/employer" className="text-violet-400 hover:underline text-sm">
            Back to Dashboard
          </Link>
        </div>
      </RequireWallet>
  ...
  const isEmployer = task.employerAddress === publicKey;

  const handleApprove = async (milestoneId: string, milestoneIndex: number) => {
    if (!publicKey || !task.escrowContractId || busy) return;
    setBusy(true);
    addLog(`Approving milestone ${milestoneIndex + 1}...`);
    try {
      const res = await escrow.approve({
  ...
  const handleRelease = async (milestoneId: string, milestoneIndex: number) => {
    ...
    try {
      const res = await escrow.release({
        contractId: task.escrowContractId,
        milestoneIndex: String(milestoneIndex),
  ...
  const handleSubmitWork = async (milestoneId: string, milestoneIndex: number) => {
    ...
    try {
      ...
      if (task.status === "claimed" || task.status === "open") {
        updateTask(task.id, { status: "in_progress", agentAddress: publicKey });
      }
      refresh();
    } catch (err) {
      addLog(`Error: ${err instanceof Error ? err.message : String(err)}`);
  ...
  const progressPct = Math.round((completedCount / task.milestones.length) * 100);
        ...
        <div className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/50 mb-8">
          ...
          <div className="flex items-center gap-6 text-sm mb-4">
            ...
            {task.escrowContractId && (
              <span className="text-xs text-violet-400 font-mono">
                Escrow: {task.escrowContractId.slice(0, 14)}...
              </span>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between text-xs text-zinc-500 mb-2">
              ...
              <span>
                {completedCount}/{task.milestones.length} ({progressPct}%)
              </span>
            </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border capitalize ${statusBadge(m.status)}`}
                    >
                      {m.status.replace("_", " ")}
                    </span>
                  </div>
        ...
        {task.status === "paid" && (
          <div className="mt-8 p-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 text-center">
            ...
            <p className="text-sm text-zinc-500">
              {task.totalAmount} {task.asset} paid out via Trustless Work escrow on Stellar.
            </p>
          </div>
        )}
